...
sub cgi_main {
	...
	if (defined $search_terms) {
        ...
        if (!legal_login($username) || !legal_password($password)) { 
            print $last_error;
            print new_account_form();
        } else {
...
sub new_account_button { 
    return <<eof;
    <p>
        <form>
            <input type="submit" name="new_account" value="Create New Account">
        </form>
    <p>
eof
}
...
sub new_account_form { 
    ...
    <form>
        ...
        Full Name: <input type="text" name="f_name" size=16></input>
        Street: <input type="text" name="street" size=16></input>
        City/Suburb: <input type="text" name="city_sub" size=16></input>
        State: <input type="text" name="state" size=16></input>
        Postcode: <input type="text" name="pc" size=16></input>
        Email Address: <input type="text" name="e_addr" size=16></input>
        <input type="submit" name="action" value="Create Account"> 
        Note: username will be used as login..
    </form>
    <pre>
    <p>
eof
...
sub create_user_file { 
    ...
    if (check_user_exists($username) == 0) { 
        ...
        open (NEW_USER, ">users/$username") or die "$!\n";
        $user_details[0] = "password=".param('password');
        $user_details[1] = "name=".param('f_name');
        $user_details[2] = "street=".param('street');
        $user_details[3] = "city=".param('city_sub');
        $user_details[4] = "state=".param('state');
        $user_details[5] = "postcode=".param('pc');
        $user_details[6] = "email=".param('e_addr');
        foreach $i (0..6) { 
            print NEW_USER $user_details[$i], "\n";
        }
        close NEW_USER;
		$flag = 1;
    }
